...
if __name__ == "__main__":
    ...
    trained_models=['gpt2-neox-untrained','gpt2-neox-1M','gpt2-neox-10M','gpt2-neox-100M','gpt2-neox-1B']
    gpt2_model='gpt2'
    ...
    cmap_all=matplotlib.colormaps.get_cmap('viridis')
    ...
    # plot gpt2 score
    ax.plot(8e9, scr_gpt2.values[0][0], color=(.3,.3,.3,1), linewidth=2, marker='o', markersize=10, zorder=2)
    ax.errorbar(8e9, scr_gpt2.values[0][0], yerr=scr_gpt2.values[0][1], color='k', zorder=1)
    # plot untrained score
    ax.plot(.8e5, scr_untrained.values[0][0], color=all_col[0, :], linewidth=2, marker='o',markeredgecolor='w',markersize=10, zorder=2)
    ax.errorbar(.8e5, scr_untrained.values[0][0], yerr=scr_untrained.values[0][1],color='k', zorder=1)
    ax.set_xticks(np.concatenate([major_ticks,[8e9]]))
    ax.xaxis.grid(True, which="major", ls="-", color='0.9', zorder=0)
    ax.yaxis.grid(False)
    ax.set_xticklabels(['untrained', '1M', '10M', '100M', '1B', 'Schrimpf\n(2021)'], rotation=0)
    ax.set_ylim(ylims)
    ax.set_xlim((.5e5, 1.5e10))
